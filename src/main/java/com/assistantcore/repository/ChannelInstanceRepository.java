...
public interface ChannelInstanceRepository extends JpaRepository<ChannelInstance, UUID> {
  ...
  @EntityGraph(attributePaths = {"tenant", "aiProfile"})
  Optional<ChannelInstance> findDetailedByInstanceName(String instanceName);
  Optional<ChannelInstance> findFirstByTenantIdOrderByCreatedAtDesc(UUID tenantId);
}
